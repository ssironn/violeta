...
function parseBlock(block: string): JSONContent[] {
  ...
  // Heading commands (\section, \subsection, etc.) — import as headings
  ...
  if (headingMatch) {
    ...
    // Skip optional [short title] argument if present
    let braceStart = headingMatch[0].length - 1
    if (headingMatch[3] === '[') {
      const closeBracket = trimmed.indexOf(']', braceStart)
      if (closeBracket !== -1) {
        braceStart = trimmed.indexOf('{', closeBracket)
        if (braceStart === -1) return []
      }
    }
    const group = extractBraceGroup(trimmed, braceStart)
    const content = parseInline(group.content)
    const attrs: Record<string, any> = { level }
    if (starred) attrs.starred = true
    return content.length > 0 ? [{ type: 'heading', attrs, content }] : []
  }

  // Font-size heading pattern: {\LARGE \textbf{...}} — generated by the editor
  const fontHeadingMatch = trimmed.match(/^\{\\(LARGE|Large|large|normalsize)\s+\\textbf\{/)
  if (fontHeadingMatch) {
    const sizeCmd = fontHeadingMatch[1]
    const sizeLevels: Record<string, number> = { LARGE: 1, Large: 2, large: 3, normalsize: 4 }
    const level = sizeLevels[sizeCmd] ?? 1
    // Extract the \textbf{...} content
    const textbfStart = trimmed.indexOf('\\textbf{')
    const group = extractBraceGroup(trimmed, textbfStart + 7)
    const content = parseInline(group.content)
    const attrs: Record<string, any> = { level }
    return content.length > 0 ? [{ type: 'heading', attrs, content }] : []
  }
...
function splitIntoBlocks(body: string): string[] {
  ...
  while (i < lines.length) {
    ...
    // Font-size heading pattern: {\LARGE \textbf{...}}
    if (/^\{\\(LARGE|Large|large|normalsize)\s+\\textbf\{/.test(trimmedLine)) {
      flushCurrent()
      blocks.push(trimmedLine)
      i++
      continue
